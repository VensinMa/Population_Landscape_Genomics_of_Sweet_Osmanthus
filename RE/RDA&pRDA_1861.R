...
# 读取文件
rawdata <- fread("Z:/root/workspace/186sample/1861snps/1861_snp_186sample_LD.pruned.noContig.raw.raw ")
...
fwrite(rawgeno, "1861_snp_186sample_LD.pruned.noContig.raw", 
       sep = "\t", quote = FALSE)
...
#save(gen.imp, file = "1861_gen.imp.RData")
#load("1861_gen.imp.RData")


env_data = read.csv("186sample_id_POP_lat_lon_ele_with_env_vars.csv",
                    header = T, row.names = 1)

env_data = env_data[, c("BIO2", "BIO8", "BIO9", "BIO10", "BIO12", 
                          "BIO15", "BIO17", "BIO18", "SRAD", "SOC", "PHH2O")]
env_select = as.matrix(env_data)


# 生成dbMEM
dbmem_data <- dbmem(env_data[, c("lon", "lat")])
