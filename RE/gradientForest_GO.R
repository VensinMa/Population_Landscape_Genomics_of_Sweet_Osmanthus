...
genOffsetAll <- sqrt(rowSums((All_future_xy_envs_cbind[, 3:ncol(All_future_xy_envs_cbind)] - All_grids[, 3:ncol(All_grids)])^2))
...
write.csv(Offset, "Genetic_Offset/ssp245_2041_2060_genetic_offset1111.csv", quote=F, row.names=F)
...
for (period in periods) {
  ...
  colnames(Offset)[3] <-"offset"
  # 保存遗传偏移结果为CSV文件
  output_file_name <- paste0("Genetic_Offset/", period, "_genetic_offset_11ev.csv")
...
write.csv(reverseOffsetGF, paste0("./future_GISS-E2-1-G_126_2080_reverseOffsetGF_19bio.csv"), row.names=FALSE) # 保存结果为CSV文件
