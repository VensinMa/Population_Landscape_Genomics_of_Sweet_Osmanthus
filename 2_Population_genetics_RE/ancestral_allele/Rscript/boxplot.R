...
# 5. 计算 count_1 / total 的比值：一个简单的数学操作，但能揭示深远的意义
merged_data$pa <- merged_data$count_1 / 1617148  # 分母是总值，固定数值确保比例一致

# 6. 按 lineage 计算均值并排序：让数据按照贡献大小排排坐，吃果果
lineage_means <- merged_data %>%
  group_by(lineage) %>%
  summarise(mean_pa = mean(pa, na.rm = TRUE)) %>%  # 忽略 NA 值，计算平均值
  arrange(mean_pa)  # 按均值升序排列
...
p <- ggplot(merged_data, aes(x = lineage, y = pa, fill = lineage)) +
  geom_boxplot(colour = "black",alpha = 0.8) +  # 使用箱线图展示数据分布
  scale_fill_manual(values = fill_colors) +  # 应用自定义颜色
  labs(
    title = "pa by Lineage (Excluding Outgroup)",  # 图表标题
    #x = "Lineage",  # x 轴标签
    y = "LOF"  # y 轴标签
  ) +
...
p <- ggplot(merged_data, aes(x = lineage, y = pa, fill = lineage)) +
  ...
  labs(
    title = expression("Derived alleles (" * italic(p)[a] * ")"), # 图标题
    x = NULL,  # 去掉 x 轴标题
    y = "LOF"  # y 轴标题
  ) +
  #scale_y_continuous(limits = c(0, max(merged_data$pa))) +  # 调整 y 轴范围，可以不显示显著性部分
  theme(
...
# ggsave("C:/Users/mawenxin/workspace/est-sfs/boxplot_without_outgroup.png", 
