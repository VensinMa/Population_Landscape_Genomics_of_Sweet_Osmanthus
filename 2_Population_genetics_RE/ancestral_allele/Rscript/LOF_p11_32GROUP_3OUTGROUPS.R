...
# 4. 计算比例：将 hom1 除以 total，得到每个样本的比例
data$p11 <- data$hom1 / 808574  # 直接按列进行数学运算

# 5. 按群体计算均值：分组计算每个群体的 p11 均值
group_means <- data %>%
  group_by(group) %>%  # 按 group 分组
  summarise(mean_p11 = mean(p11, na.rm = TRUE))  # 计算每组的平均值，na.rm = TRUE 忽略 NA 值

# 6. 按均值排序群体：根据 mean_p11 排序群体的顺序
group_means <- group_means %>%
  arrange(mean_p11)  # 按 mean_p11 升序排列
...
# 绘制 箱线图 + 散点图
p = ggplot(data, aes(x = group, y = p11, fill = lineage)) +
  # 添加箱线图
  geom_boxplot(
    width = 0.8,  # 箱线图宽度
...
# 10. 保存图片
pdf(file = "LOF_p11_32GROUP_3OUTGROUPS.pdf", height = 5, width = 7.5,)
print(p)
dev.off()
