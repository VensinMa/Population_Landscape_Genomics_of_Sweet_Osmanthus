...
# 5. 计算 het / total 的比值：一个简单的数学操作，但能揭示深远的意义
merged_data$p01 <- merged_data$het / 808574  # 分母是总值，固定数值确保比例一致

# 6. 按 lineage 计算均值并排序：让数据按照贡献大小排排坐，吃果果
lineage_means <- merged_data %>%
  group_by(lineage) %>%
  summarise(mean_p01 = mean(p01, na.rm = TRUE)) %>%  # 忽略 NA 值，计算平均值
  arrange(mean_p01)  # 按均值升序排列
...
p <- ggplot(merged_data, aes(x = lineage, y = p01, fill = lineage)) +
  geom_boxplot(colour = "black",alpha = 0.8) +  # 使用箱线图展示数据分布
  scale_fill_manual(values = fill_colors) +  # 应用自定义颜色
  labs(
    title = "p01 by Lineage (Excluding Outgroup)",  # 图表标题
    #x = "Lineage",  # x 轴标签
    y = "DEL"  # y 轴标签
  ) +
...
p <- ggplot(merged_data, aes(x = lineage, y = p01, fill = lineage)) +
  ...
  #scale_y_continuous(limits = c(0, max(merged_data$p01))) +  # 调整 y 轴范围，可以不显示显著性部分
  theme(
...
pdf(file = "DEL_p01_4LIN_3_outgroup.pdf", height = 6, width = 4,)
print(p)
dev.off()
